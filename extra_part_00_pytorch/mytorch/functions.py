...
from mytorch import Function, Mytorch, as_mytorch, as_array
...
class GetItemGrad(Function):
    ...
    def forward(self, gy):
        xp = mytorch.cuda.get_array_module(gy)
...
def mean_squared_error_simple(x0, x1):
    x0, x1 = as_mytorch(x0), as_mytorch(x1)
    diff = x0 - x1
...
def accuracy(y, t):
    ...
    """
    y, t = as_mytorch(y), as_mytorch(t)

    pred = y.data.argmax(axis=1).reshape(t.shape)
    result = (pred == t.data)
    acc = result.mean()
    return Mytorch(as_array(acc))
...
# from mytorch import add
# from mytorch import sub
# from mytorch import rsub
# from mytorch import mul
# from mytorch import div
# from mytorch import neg
# from mytorch import pow